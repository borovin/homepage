...
jsx.CallBacks = new function () {
    ...
    this.dispatch = function (type, object, event, query) {
        jsx.Console.group('Type: ', type + ', object: ', object, ['jsx/CallBacks', type]);
     ...
     */
    this.need = function(which, listener) {
        this.neededComponents.push({
            dependencies: which,
            listener: listener
        });
        this.checkNeeded();
    ...
    this.checkNeeded = function() {
        ...
        for (var n = 0, ln = this.neededComponents.length; n < ln; n++) {
            for (var r = 0, lr = this.readyComponents.length; r < lr; r++) {
                var index = this.neededComponents[n].dependencies.indexOf(this.readyComponents[r].name);
                if (index !== -1) {
                    var requester = this.neededComponents[n];
                    var requested = this.readyComponents[r];

                    requester.dependencies.splice(index, 1);
                    if (requester.dependencies.length === 0) {
